...
from model_utils import YoloeV26Lightning,ImageTrackingCallback
import lightning as L



# Create models directory (if it doesn't already exist), see: https://docs.python.org/3/library/pathlib.html#pathlib.Path.mkdir
MODEL_PATH = Path("models_checkpoints/yoloe26")
...
logger = TensorBoardLogger(save_dir="tensorboard_logs",name="yoloev26_linearprobing_skadi")


class_names =  ['car', 'truck', 'van_minivan']

###since we are training whole detection_head then set learning rate as 3*1e-5
model = YoloeV26Lightning(text_prompt=class_names,lr=3*1e-4,weights_path="models/yoloe-26l-seg.pt",conf_thres=0.2,only_cv3=False)
...
checkpoint_callback = ModelCheckpoint(dirpath=MODEL_PATH,
                                      monitor='val/mAP_50',
                                      filename='yoloe26-linear-probing-skadi-ctzn-objects-lr1e-4-{epoch}-{val/mAP_50:.2f}-{val/Recall_100:.2f}',
                                      mode='max',
