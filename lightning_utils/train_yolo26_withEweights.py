...
from model_utils import Yolo26Lightning,ImageTrackingCallback
import lightning as L



# Create models directory (if it doesn't already exist), see: https://docs.python.org/3/library/pathlib.html#pathlib.Path.mkdir
MODEL_PATH = Path("models_checkpoints/yolo26_withEweights")
...
logger = TensorBoardLogger(save_dir="tensorboard_logs",name="yolov26_withEweights_linearprobing_skadi")


class_names =  ['car', 'truck', 'van_minivan']

###since we are training whole detection_head then set learning rate as 3*1e-5
model = Yolo26Lightning(nc = len(class_names),scale = "l",lr = 3*1e-4,weights_path="models/yoloe-26l-seg.pt",only_cv3=False,conf_thres=0.2)
...
checkpoint_callback = ModelCheckpoint(dirpath=MODEL_PATH,
                                      monitor='val/mAP_50',
                                      filename='yolo26-withEweights-linear-probing-skadi-ctzn-objects-lr1e-4-{epoch}-{val/mAP_50:.2f}-{val/Recall_100:.2f}',
                                      mode='max',
