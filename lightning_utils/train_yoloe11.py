...
from model_utils import Yoloe11Lightning,ImageTrackingCallback
import lightning as L



# Create models directory (if it doesn't already exist), see: https://docs.python.org/3/library/pathlib.html#pathlib.Path.mkdir
MODEL_PATH = Path("models_checkpoints/yoloe11")
...
logger = TensorBoardLogger(save_dir="tensorboard_logs",name="yoloe11_linearprobing_skadi")
...
model = Yoloe11Lightning(text_prompt=class_names,lr=3*1e-4,weights_path="models/yoloe-11l-seg.pt",only_cv3=False)

# 1. Define the callback
early_stop_callback = EarlyStopping(
    monitor="val/mAP_50",  # Metric to monitor
    min_delta=0.00,      # Minimum change to qualify as an improvementa
...
checkpoint_callback = ModelCheckpoint(dirpath=MODEL_PATH,
                                      monitor='val/mAP_50',
                                      filename='yoloe11-linear-probing-skadi-ctzn-objects-lr1e-4-{epoch}-{val/mAP_50:.2f}-{val/Recall_100:.2f}',
                                      mode='max',
