...
MODEL_PATH = Path("models_checkpoints/yolo11")
...
logger = TensorBoardLogger(save_dir="tensorboard_logs",name="yolo11_linearprobing_skadi")
...
model = Yolo11Lightning(nc = len(class_names),scale = "l",lr = 3*1e-4,weights_path="models/yolo11l.pt",only_cv3=False)
...
checkpoint_callback = ModelCheckpoint(dirpath=MODEL_PATH,
                                      monitor='val/mAP_50',
                                      filename='yolo11-linear-probing-skadi-ctzn-objects-lr1e-4-{epoch}-{val/mAP_50:.2f}-{val/Recall_100:.2f}',
                                      mode='max',
