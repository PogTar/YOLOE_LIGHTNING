...
MODEL_PATH = Path("models_checkpoints/yolov8_withEweights")
...
logger = TensorBoardLogger(save_dir="tensorboard_logs",name="yolov8_withEweights_linearprobing_skadi")



class_names =  ['car', 'truck', 'van_minivan']
###Also train detection head
model = Yolov8Lighning(nc = len(class_names),scale = "l",lr = 3*1e-4,weights_path="models/yoloe-v8l-seg.pt",only_cv3=False)
...
checkpoint_callback = ModelCheckpoint(dirpath=MODEL_PATH,
                                      monitor='val/mAP_50',
                                      filename='yolov8-linear-probing-skadi-ctzn-objects-lr1e-4-{epoch}-{val/mAP_50:.2f}-{val/Recall_100:.2f}',
                                      mode='max',
