...
MODEL_PATH = Path("models_checkpoints/yolo26")
...
logger = TensorBoardLogger(save_dir="tensorboard_logs",name="yolov26_linearprobing_skadi")


class_names =  ['car', 'truck', 'van_minivan']

###since we are training whole detection_head then set learning rate as 3*1e-5
model = Yolo26Lightning(nc = len(class_names),scale = "l",lr = 3*1e-4,weights_path="models/yolo26l.pt",only_cv3=False,conf_thres=0.2)
...
checkpoint_callback = ModelCheckpoint(dirpath=MODEL_PATH,
                                      monitor='val/mAP_50',
                                      filename='yolo26-linear-probing-skadi-ctzn-objects-lr1e-4-{epoch}-{val/mAP_50:.2f}-{val/Recall_100:.2f}',
                                      mode='max',
